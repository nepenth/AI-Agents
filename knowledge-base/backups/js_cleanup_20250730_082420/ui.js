...
/**
 * Legacy Theme Manager for dark/light mode switching
 */
...
class UIManager {
    constructor() {
        ...
        this.themeManager = new LegacyThemeManager('theme-toggle');
    ...
    initializePolling() {
        ...
        if (!window.socket) {
            ...
            this.startPolling('status', '/agent/status', 10000);  // Reduced from 3s to 10s
            this.startPolling('logs', '/logs/recent', 15000);     // Reduced from 2.5s to 15s
            this.startPolling('gpu-stats', '/gpu-stats', 30000);  // Reduced from 5s to 30s
            console.log('✅ API polling started (SocketIO unavailable) - optimized intervals');
        } else {
            ...
            this.startPolling('status', '/agent/status', 30000); // Reduced from 15s to 30s (very infrequent backup)
...
class Router {
    constructor(api) {
        ...
        // Capture API reference for use in route managers
        ...
        this.routes = {
            'dashboard': {
                ...
                manager: () => {
                    ...
                    return new window.DashboardManager(apiClient);
                }
            },
            'chat': {
                title: 'Chat Interface',
                endpoint: '/v2/page/chat',
                manager: () => window.ChatManager ? new window.ChatManager(apiClient) : null
            },
            'kb': {
                title: 'Knowledge Base',
                endpoint: '/v2/page/kb',
                manager: () => window.KnowledgeBaseManager ? new window.KnowledgeBaseManager(apiClient) : null
            },
            'synthesis': {
                title: 'Synthesis Documents',
                endpoint: '/v2/page/synthesis',
                manager: () => window.SynthesisManager ? new window.SynthesisManager(apiClient) : (window.StaticPagesManager ? new window.StaticPagesManager('synthesis', apiClient) : null)
            },
            'schedule': {
                title: 'Schedules',
                endpoint: '/v2/page/schedule',
                manager: () => window.ScheduleManager ? new window.ScheduleManager(apiClient) : (window.StaticPagesManager ? new window.StaticPagesManager('schedule', apiClient) : null)
            },
            'tweets': {
                title: 'Tweet Management',
                endpoint: '/v2/page/tweets',
                manager: () => {
                    return window.TweetManagementManager ? new window.TweetManagementManager(apiClient) : null;
                }
            }
        };
...
window.LegacyThemeManager = LegacyThemeManager;
