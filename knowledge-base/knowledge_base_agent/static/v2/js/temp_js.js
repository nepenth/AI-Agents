...
class ModernKnowledgeBaseManager extends BaseManager {
    ...
    ...options
        ...
        // API / media paths
        this.apiBase = options.apiBase || '/api/kb'; // e.g., GET /api/kb/items
        ...
        try {
            ...
            if (Array.isArray(response)) {
                response.forEach(item => {
                    ...
                    if (!item.content) {
                        item.content = item.markdown_content || item.full_text || item.description || '';
                    }
    ...
    createAndShowModal(item) {
        ...
        const mediaFiles = Array.isArray(item.kb_media_paths) ? item.kb_media_paths : [];
