...
between the state and the actual knowledge base.
"""
...
from knowledge_base_agent.tweet_utils import (
    parse_tweet_id_from_url,
    load_tweet_urls_from_links,
)
...
class StateManager:
    ...
    async def initialize(self) -> None:
            ...
            try:
                async with aiofiles.open(self.unprocessed_tweets_file, "r") as f:
                    ...
                    if not isinstance(tweet_ids, list):
                        logging.error(
                            "Unprocessed tweets file contains invalid data structure, resetting"
                        )
                        tweet_ids = []
                    else:
                        tweet_ids = [
                            str(tid) for tid in tweet_ids if isinstance(tid, (str, int))
                        ]
                    self._unprocessed_tweets = tweet_ids
                logging.info(
                    f"Loaded {len(self._unprocessed_tweets)} unprocessed tweets from {self.unprocessed_tweets_file}"
                )
            except Exception as e:
                logging.error(
                    f"Error loading unprocessed tweets from {self.unprocessed_tweets_file}: {e}"
                )
                self._unprocessed_tweets = []
                try:
                    backup_path = self.unprocessed_tweets_file.with_suffix(
                        ".backup_corrupted"
                    )
                    shutil.copy2(self.unprocessed_tweets_file, backup_path)
                    logging.info(
                        f"Backed up corrupted unprocessed tweets file to {backup_path}"
                    )
                except Exception as backup_e:
                    logging.error(
                        f"Failed to backup corrupted unprocessed tweets file: {backup_e}"
                    )
            ...
            try:
                async with aiofiles.open(self.processed_tweets_file, "r") as f:
                    ...
                    if not isinstance(processed_data, dict):
                        logging.error(
                            "Processed tweets file contains invalid data structure, resetting"
                        )
                        processed_data = {}
                    else:
                ...
                logging.info(
                    f"Loaded {len(self._processed_tweets)} processed tweets from {self.processed_tweets_file}"
                )
            except Exception as e:
                logging.error(
                    f"Error loading processed tweets from {self.processed_tweets_file}: {e}"
                )
                self._processed_tweets = {}
                try:
                    backup_path = self.processed_tweets_file.with_suffix(
                        ".backup_corrupted"
                    )
                    shutil.copy2(self.processed_tweets_file, backup_path)
                    logging.info(
                        f"Backed up corrupted processed tweets file to {backup_path}"
                    )
                except Exception as backup_e:
                    logging.error(
                        f"Failed to backup corrupted processed tweets file: {backup_e}"
                    )

        initial_unprocessed_count = len(self._unprocessed_tweets)
        self._unprocessed_tweets = [
            tid for tid in self._unprocessed_tweets if tid not in self._processed_tweets
        ]
        if initial_unprocessed_count != len(self._unprocessed_tweets):
            logging.info(
                f"Removed {initial_unprocessed_count - len(self._unprocessed_tweets)} duplicates from unprocessed (already in processed) during load"
            )
            ...
            try:
                async with aiofiles.open(self.tweet_cache_file, "r") as f:
                    ...
                    if not isinstance(cache_data, dict):
                        logging.error(
                            "Tweet cache file contains invalid data structure, resetting"
                        )
                        cache_data = {}
                    self._tweet_cache = cache_data
                logging.info(
                    f"Loaded {len(self._tweet_cache)} cached tweets from {self.tweet_cache_file}"
                )
            except Exception as e:
                logging.error(
                    f"Error loading tweet cache from {self.tweet_cache_file}: {e}"
                )
                self._tweet_cache = {}
                try:
                    ...
                    logging.info(
                        f"Backed up corrupted tweet cache file to {backup_path}"
                    )
                except Exception as backup_e:
                    logging.error(
                        f"Failed to backup corrupted tweet cache file: {backup_e}"
                    )
        ...
        try:
            ...
            if self.bookmarks_file.exists():
                try:
                    async with aiofiles.open(
                        self.bookmarks_file, "r", encoding="utf-8"
                    ) as f:
                        content = await f.read()
                    bookmarks_data = json.loads(content) if content.strip() else {}
                    if not isinstance(bookmarks_data, dict):
                        logging.error(
                            f"Bookmarks file {self.bookmarks_file} is not a valid JSON object, treating as empty."
                        )
                        bookmarks_data = {}
                except Exception as e:
                    logging.error(
                        f"Error loading or parsing bookmarks file {self.bookmarks_file}: {e}"
                    )
                    bookmarks_data = {}
                ...
                for tweet_id_key in bookmarks_data.keys():
                    if isinstance(tweet_id_key, str) and tweet_id_key.isdigit():
                        ...
                        if parsed_id:
                            ...
                            logging.warning(
                                f"Could not extract a valid tweet ID from bookmark key: '{tweet_id_key}'"
                            )
                logging.info(
                    f"Found {len(bookmarked_tweet_ids_to_consider)} unique tweet IDs in bookmarks file."
                )
            else:
                logging.info(
                    f"Bookmarks file not found at {self.bookmarks_file}, skipping proactive bookmark check."
                )
            ...
            for tweet_id in bookmarked_tweet_ids_to_consider:
                if (
                    ...
                    logging.debug(
                        f"Proactively added tweet {tweet_id} from bookmarks.json to unprocessed queue."
                    )
                    if tweet_id not in self._tweet_cache:
                        logging.debug(
                            f"Tweet {tweet_id} from bookmarks not in cache. Initializing basic cache entry."
                        )
                        await self.initialize_tweet_cache(
                            tweet_id, {"tweet_id": tweet_id, "source": "bookmark_init"}
                        )

            if newly_added_from_bookmarks > 0:
                logging.info(
                    f"Proactively added {newly_added_from_bookmarks} tweets from bookmarks.json to the unprocessed queue."
                )
            else:
                logging.info(
                    "No new tweets from bookmarks.json needed to be added to the unprocessed queue at this stage."
                )

        except Exception as e:
            logging.error(
                f"Error during proactive check of tweet_bookmarks.json: {e}",
                exc_info=True,
            )

        logging.info(
            "Validating tweet cache integrity (KB item paths and existence)..."
        )
        await self.validate_kb_items()  # Uses self.config for path resolution

        logging.info(
            "Reconciling tweet states across processed, unprocessed, and cached lists..."
        )
        ...
        for tweet_id in processed_ids_copy:
            if tweet_id not in self._tweet_cache:
                logging.warning(
                    f"Tweet {tweet_id} is in processed list but not in tweet_cache. Moving to unprocessed for full processing."
                )
        ...
        for tweet_id in cached_ids_copy:
            tweet_data_before_validation = copy.deepcopy(
                self._tweet_cache.get(tweet_id, {})
            )  # Get a copy before validation

            if not tweet_data_before_validation:
                logging.warning(
                    f"Tweet {tweet_id} key exists in cache but data is null during validation. Skipping."
                )
                continue

            summary_stats["total_tweets_validated"] += 1
            is_fully_processed_and_valid = (
                await self._validate_tweet_state_comprehensive(
                    tweet_id, tweet_data_before_validation
                )
            )  # Pass the copy

            # Update summary stats based on the final state of the tweet_data after validation
            final_tweet_data = self._tweet_cache.get(
                tweet_id, {}
            )  # Re-fetch the (potentially modified) data

            if is_fully_processed_and_valid:
                summary_stats["fully_valid_and_complete"] += 1
                if tweet_id not in self._processed_tweets:
                    self._processed_tweets[tweet_id] = datetime.now(
                        timezone.utc
                    ).isoformat()
                    logging.info(
                        f"Tweet {tweet_id} validated as fully processed by comprehensive check, ensuring it is in processed list."
                    )
                    tweets_to_mark_processed.add(tweet_id)
                if tweet_id in self._unprocessed_tweets:
                    self._unprocessed_tweets.remove(tweet_id)
                    logging.debug(
                        f"Removed comprehensively validated tweet {tweet_id} from unprocessed list."
                    )
            else:
                ...
                current_categories_processed = final_tweet_data.get(
                ...
                original_attempts = tweet_data_before_validation.get(
                    "recategorization_attempts", 0
                )
                final_attempts = final_tweet_data.get("recategorization_attempts", 0)
                if (
                    not current_categories_processed
                    and final_attempts > original_attempts
                ):
                ...
                elif final_tweet_data.get(
                    "kb_item_created", False
                ):  # kb_item_created is true
                    kb_path_rel = final_tweet_data.get("kb_item_path", "")
                    kb_readme_abs = (
                        self.config.resolve_path_from_project_root(kb_path_rel)
                        if kb_path_rel
                        else None
                    )
                    if not kb_path_rel or not (
                        kb_readme_abs and kb_readme_abs.exists()
                    ):
                        summary_stats["kb_item_path_issues"] += 1

                if tweet_id in self._processed_tweets:
                    logging.warning(
                        f"Tweet {tweet_id} was in processed list but failed comprehensive validation. Moving to unprocessed."
                    )
                    del self._processed_tweets[tweet_id]
                    self.validation_fixes += 1
                ...
                if tweet_id not in self._unprocessed_tweets:
                    self._unprocessed_tweets.append(tweet_id)
                    logging.debug(
                        f"Ensured tweet {tweet_id} (failed comprehensive validation) is in unprocessed list."
                    )
                tweets_to_process.add(tweet_id)
        ...
        for tweet_id in self._tweet_cache:
            if (
                tweet_id not in self._processed_tweets
                and tweet_id not in self._unprocessed_tweets
            ):
                logging.info(
                    f"Tweet {tweet_id} from cache wasn't assigned to processed/unprocessed by prior stages. Adding to unprocessed as default."
                )
                self._unprocessed_tweets.append(tweet_id)
                tweets_to_process.add(tweet_id)

        processed_in_unprocessed = set(self._processed_tweets.keys()).intersection(
            set(self._unprocessed_tweets)
        )
        if processed_in_unprocessed:
            logging.warning(
                f"Found {len(processed_in_unprocessed)} tweets in both processed and unprocessed lists. Removing from processed: {processed_in_unprocessed}"
            )
        ...
        for tweet_id, tweet_data in self._tweet_cache.items():
            if tweet_data.get("kb_item_created", False):
                kb_path_rel_project_str = tweet_data.get(
                    "kb_item_path", ""
                )  # Path to README.md relative to project_root
                if kb_path_rel_project_str:
                    kb_readme_abs_path = self.config.resolve_path_from_project_root(
                        kb_path_rel_project_str
                    )
                    # For this check, we need the directory containing README.md
                    kb_item_dir_abs_path = kb_readme_abs_path.parent
                    if (
                        kb_item_dir_abs_path.exists() and kb_readme_abs_path.exists()
                    ):  # Check dir and README
                        # We store the KB *item directory path* (relative to project root) for valid_kb_paths set for orphan check
                        kb_item_dir_rel_project_str = str(
                            self.config.get_relative_path(kb_item_dir_abs_path)
                        )
                        valid_kb_paths_rel_project.add(kb_item_dir_rel_project_str)
                        if tweet_id in tweet_to_path_rel_project:
                            duplicates[tweet_id] = [
                                tweet_to_path_rel_project[tweet_id],
                                kb_item_dir_rel_project_str,
                            ]
                        else:
                            tweet_to_path_rel_project[tweet_id] = (
                                kb_item_dir_rel_project_str
                        ...
                        logging.debug(
                            f"Tweet {tweet_id} kb_item_created but path invalid/missing: {kb_path_rel_project_str} (resolved to {kb_readme_abs_path})"
                        )
                        tweet_data["kb_item_created"] = False
                        # tweet_data['kb_item_path'] = '' # Don't clear yet, might be useful for debugging
                        await self.update_tweet_data(
                            tweet_id, tweet_data
                        )  # Persist kb_item_created = False
                else:
                    logging.debug(
                        f"Tweet {tweet_id} kb_item_created but kb_item_path is empty."
                    )
                    tweet_data["kb_item_created"] = False
                    await self.update_tweet_data(tweet_id, tweet_data)

        logging.info(
            f"Found {len(valid_kb_paths_rel_project)} valid KB item directories (relative to project root) in tweet_cache.json"
        )
        if duplicates:
            logging.info(
                f"Found {len(duplicates)} tweets with duplicate KB item directory paths (relative to project root)"
            )
        ...
        for readme_abs_path in kb_dir_abs.rglob("README.md"):
            ...
            if kb_item_dir_abs != kb_dir_abs:
                all_kb_item_dirs_rel_project.add(
                    str(self.config.get_relative_path(kb_item_dir_abs))
                )

        logging.info(
            f"Found {len(all_kb_item_dirs_rel_project)} KB item directories (relative to project root) by scanning {kb_dir_abs}"
        )
        ...
        for tweet_id, paths_rel_project in duplicates.items():
            ...
            path_to_move_rel = [
                p for p in paths_rel_project if p != tweet_to_path_rel_project[tweet_id]
            ][0]
            path_to_move_abs = self.config.resolve_path_from_project_root(
                path_to_move_rel
            )
            quarantine_path_abs = (
                quarantine_dir_abs
                / f"duplicate_{tweet_id}_{Path(path_to_move_rel).name}_{timestamp}"
            )
            logging.debug(
                f"Moving duplicate KB item for tweet {tweet_id} from {path_to_move_abs} to quarantine {quarantine_path_abs}"
            )
            try:
                if path_to_move_abs.exists():
                    shutil.move(str(path_to_move_abs), str(quarantine_path_abs))
            except Exception as e:
                logging.error(
                    f"Failed to move duplicate KB item {path_to_move_abs} to quarantine: {e}"
                )

        for orphan_path_rel_project in orphans_rel_project:
            orphan_path_abs = self.config.resolve_path_from_project_root(
                orphan_path_rel_project
            )
            quarantine_path_abs = (
                quarantine_dir_abs
                / f"orphan_{Path(orphan_path_rel_project).name}_{timestamp}"
            )
            logging.debug(
                f"Moving orphaned KB item from {orphan_path_abs} to quarantine {quarantine_path_abs}"
            )
            try:
                if orphan_path_abs.exists():
                    shutil.move(str(orphan_path_abs), str(quarantine_path_abs))
            except Exception as e:
                logging.error(
                    f"Failed to move orphaned KB item {orphan_path_abs} to quarantine: {e}"
                )

        logging.info(
            f"Moved {len(orphans_rel_project)} orphaned KB items and handled {len(duplicates)} duplicates to quarantine"
        )

        # Save state files if changes were made during reconciliation
        if (
            len(self._unprocessed_tweets) != initial_unprocessed_count_recon
            or len(self._processed_tweets) != initial_processed_count_recon
            or tweets_to_process
            or tweets_to_mark_processed
            or duplicates
            or orphans_rel_project
            or self.validation_fixes > 0
        ):
            await self._atomic_write_json(
                list(set(self._unprocessed_tweets)), self.unprocessed_tweets_file
            )
            await self._atomic_write_json(
                self._processed_tweets, self.processed_tweets_file
            )
            ...
            logging.info(
                f"Reconciliation results: {len(tweets_to_mark_processed)} marked/confirmed processed, {len(tweets_to_process)} marked/confirmed unprocessed. State files saved."
            )
        ...
        try:
            ...
            logging.error(
                f"Failed to synchronize KB items to database during initialization: {e}"
            )

        self._initialized = True
        logging.info(
            f"StateManager initialization complete. Validation fixes: {self.validation_fixes}. Final state: {len(self._unprocessed_tweets)} unprocessed, {len(self._processed_tweets)} processed."
        )

        # Log summary statistics
        logging.info("--- Tweet Data Validation Summary (on startup) ---")
        logging.info(
            f"Total tweets validated: {summary_stats['total_tweets_validated']}"
        )
        logging.info(
            f"  Fully valid & complete: {summary_stats['fully_valid_and_complete']}"
        )
        logging.info(
            f"  Tweets needing cache completion (media download/verify): {summary_stats['needs_cache_completion']}"
        )
        logging.info(
            f"  Tweets needing media processing (e.g., alt text generation): {summary_stats['needs_media_processing']}"
        )
        logging.info(
            f"  Tweets needing categorization (general): {summary_stats['needs_categorization']}"
        )
        if summary_stats["marked_for_recategorization"] > 0:  # Only show if relevant
            logging.info(
                f"    Specifically marked for re-categorization: {summary_stats['marked_for_recategorization']}"
            )
        logging.info(
            f"  Tweets needing KB item creation: {summary_stats['needs_kb_item_creation']}"
        )
        logging.info(
            f"  Tweets with KB item path issues (e.g. README missing): {summary_stats['kb_item_path_issues']}"
        )
        logging.info("--------------------------------------------------")
    ...
    async def _atomic_write_json(self, data: Any, filepath: Path) -> None:
        ...
        try:
            ...
            temp_fd, temp_path_str = tempfile.mkstemp(
                dir=filepath.parent, text=True
            )  # text=True for string ops
            os.close(temp_fd)  # Close file descriptor from mkstemp
            temp_file = Path(temp_path_str)
            async with aiofiles.open(temp_file, "w", encoding="utf-8") as f:
                await f.write(json.dumps(data, indent=2))
            shutil.move(
                str(temp_file), str(filepath)
            )  # Use shutil.move for atomicity if possible
    ...
    async def mark_tweet_processed(
        self, tweet_id: str, tweet_data: Dict[str, Any] = None
    ) -> None:
        """
        ...
        async with self._lock:
            try:
                if not tweet_data:
                    logging.warning(
                        f"No tweet data provided for {tweet_id} to validate before marking processed, skipping mark as processed"
                    )
                ...
                if tweet_id in self._processed_tweets:
                    logging.debug(
                        f"Tweet {tweet_id} already marked as processed, skipping"
                    )
                    return
                ...
                kb_item_abs_path = (
                    self.config.resolve_path_from_project_root(kb_item_path_rel)
                    if kb_item_path_rel
                    else None
                )
                ...
                required_checks = [
                    ...
                    tweet_data.get(
                        ...
                        not bool(
                            ...
                            or tweet_data.get("all_downloaded_media_for_thread", [])
                        ),
                    ),
                    tweet_data.get("categories_processed", False),
                    tweet_data.get("kb_item_created", False),
                    kb_item_path_rel is not None
                    and (kb_item_abs_path.exists() if kb_item_abs_path else False),
                ]
                ...
                if not all(required_checks):
                    ...
                    logging.warning(
                        f"Tweet {tweet_id} not fully processed or valid for marking. Missing/Invalid steps: {', '.join(filter(None, missing_steps))}. KB Path checked: {kb_item_abs_path}"
                    )
                    return
                ...
                if tweet_id in self._unprocessed_tweets:
                    ...
                    logging.debug(
                        f"Tweet {tweet_id} not in unprocessed list (or already removed)"
                    )

                await self._atomic_write_json(
                    list(self._unprocessed_tweets), self.unprocessed_tweets_file
                )
                await self._atomic_write_json(
                    self._processed_tweets, self.processed_tweets_file
                )
                logging.info(
                    f"Marked tweet {tweet_id} as fully processed; unprocessed remaining: {len(self._unprocessed_tweets)}"
                )

            except Exception as e:
                logging.error(f"Failed to mark tweet {tweet_id} as processed: {e}")
                if (
                    tweet_id not in self._unprocessed_tweets
                ):  # Ensure it goes back if error during marking
                    self._unprocessed_tweets.append(tweet_id)
                # Consider if _processed_tweets should be rolled back for this ID if error occurs after adding.
                if tweet_id in self._processed_tweets:
                    del self._processed_tweets[tweet_id]
                raise StateError(
                    f"Failed to update processing state for {tweet_id}: {e}"
                )
    ...
    async def get_unprocessed_tweets(self) -> List[str]:
        ...
        logging.debug(
            f"Returning {len(self._unprocessed_tweets)} unprocessed tweet IDs"
        )
        ...
        try:
            ...
            async with self._lock:
                ...
                for tid in valid_ids:
                    if (
                        tid not in self._processed_tweets
                        and tid not in current_unprocessed_set
                    ):
                        self._unprocessed_tweets.append(tid)
                        current_unprocessed_set.add(tid)
                        newly_added_count += 1
                        logging.debug(
                            f"Added tweet {tid} from bookmarks to unprocessed list."
                        )

                if newly_added_count > 0:
                    await self.save_unprocessed()  # Save if changes were made
                    logging.info(
                        f"Added {newly_added_count} new tweets to process from bookmarks: {self.config.bookmarks_file}"
                    )
                else:
                    logging.info(
                        f"No new tweets from bookmarks {self.config.bookmarks_file} to add."
                    )
        except Exception as e:
    ...
    async def save_unprocessed(self) -> None:
        ...
        try:
            ...
            logging.debug(
                f"Saved {len(self._unprocessed_tweets)} unprocessed tweets to {self.unprocessed_tweets_file}"
            )
        except Exception as e:
    ...
    async def get_processing_state(self, tweet_id: str) -> Dict[str, bool]:
        ...
        try:
            tweet_data = await self.get_tweet(
                tweet_id
            )  # Fetches from self._tweet_cache
            if not tweet_data:
                logging.debug(
                    f"No processing state found for tweet {tweet_id} (not in cache)"
                )
                return {
                    "fully_processed": tweet_id in self._processed_tweets
                }  # Basic check if not in cache

            kb_item_path_rel = tweet_data.get("kb_item_path")
            kb_item_exists = False
            if kb_item_path_rel:
                kb_item_abs_path = self.config.resolve_path_from_project_root(
                    kb_item_path_rel
                )
            ...
            state = {
                ...
                and kb_item_exists,  # Check existence too
                "fully_processed": tweet_id in self._processed_tweets,
            }
    ...
    async def update_tweet_data(
        ...
        try:
            await self._atomic_write_json(self._tweet_cache, self.tweet_cache_file)
            logging.debug(
                f"Updated tweet data for {tweet_id} in cache file: {self.tweet_cache_file}"
            )
        except Exception as e:
    ...
    async def save_tweet_cache(self, tweet_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            await self._atomic_write_json(self._tweet_cache, self.tweet_cache_file)
            logging.debug(
                f"Saved tweet {tweet_id} to cache file: {self.tweet_cache_file}"
            )
        except Exception as e:
    ...
    async def verify_cache_status(self) -> List[str]:
        ...
        for tweet_id in self._unprocessed_tweets:
            ...
            if not cached_data or not cached_data.get("cache_complete", False):
                ...
                logging.debug(
                    f"Tweet {tweet_id} needs cache update (cache_complete is false or no data)."
                )
        logging.info(
            f"Verified cache status: {len(tweets_needing_cache)} tweets need caching based on 'cache_complete' flag."
        )
        return tweets_needing_cache

    async def update_media_analysis(
        self, tweet_id: str, media_analysis: Dict[str, Any]
    ) -> None:
        if tweet_id not in self._tweet_cache:
            raise StateError(
                f"Tweet {tweet_id} not found in cache for media analysis update"
            )
        self._tweet_cache[tweet_id].update(
            {"media_analysis": media_analysis, "media_analysis_complete": True}
        )
        await self._atomic_write_json(self._tweet_cache, self.tweet_cache_file)
        logging.debug(f"Updated media analysis for tweet {tweet_id}")

    async def update_categories(
        self, tweet_id: str, category_data: Dict[str, Any]
    ) -> None:
        if tweet_id not in self._tweet_cache:
            raise StateError(f"Tweet {tweet_id} not found in cache for category update")
        self._tweet_cache[tweet_id].update(
            {
                "main_category": category_data.get(
                    "main_category"
                ),  # Store directly in tweet_data
                "sub_category": category_data.get("sub_category"),
                "item_name_suggestion": category_data.get(
                    "item_name_suggestion"
                ),  # Use 'item_name_suggestion' consistent with ContentProcessor
    ...
    async def mark_kb_item_created(
        ...
        try:
            ...
            if tweet_id not in content_sample:
                logging.warning(
    ...
    async def mark_media_processed(self, tweet_id: str) -> None:
        ...
        if tweet_data:
            ...
            logging.warning(f"Tweet {tweet_id} not found to mark media processed.")
    ...
    async def mark_categories_processed(self, tweet_id: str) -> None:
        ...
        if tweet_data:
            ...
            logging.warning(f"Tweet {tweet_id} not found to mark categories processed.")

    async def initialize_tweet_cache(
        self, tweet_id: str, tweet_data: Dict[str, Any]
    ) -> None:
        """Initialize new tweet in cache. Paths in tweet_data should be relative if applicable."""
        if tweet_id in self._tweet_cache:
            logging.debug(
                f"Tweet {tweet_id} already in cache, updating with: {tweet_data}"
            )
        else:
            logging.debug(
                f"Initializing cache for new tweet {tweet_id} with: {tweet_data}"
            )
        ...
        base_data = {
            ...
            "all_downloaded_media_for_thread": tweet_data.get(
            ...
            "display_title": None,
            "source": tweet_data.get(
                "source", "unknown"
            ),  # e.g., 'bookmark_init', 'playwright_fetch'
        }
        ...
        if (
            "categories" in tweet_data
            and isinstance(tweet_data["categories"], dict)
            and tweet_data["categories"]
        ):
            if set(tweet_data["categories"].keys()) & {
                "main_category",
                "sub_category",
                "item_name",
            }:
                ...
                if "categories_processed" in tweet_data:
                    base_data["categories_processed"] = tweet_data[
        ...
        if (
            "kb_item_created" in tweet_data
            and tweet_data["kb_item_created"]
            and "kb_item_path" in tweet_data
            and tweet_data["kb_item_path"]
        ):
        ...
        if tweet_id in self._tweet_cache:
            ...
            if (
                "categories" in existing_data
                and "categories_processed" in existing_data
                and existing_data["categories_processed"] is True
                and isinstance(existing_data["categories"], dict)
                and all(
                    key in existing_data["categories"]
                    for key in ["main_category", "sub_category", "item_name"]
                )
            ):
                ...
                base_data["item_name_suggestion"] = existing_data[
                    "item_name_suggestion"
                ]
                base_data["categories_processed"] = existing_data[
                    "categories_processed"
                ]
    ...
    async def mark_tweet_unprocessed(self, tweet_id: str) -> None:
        ...
        async with self._lock:
            if tweet_id in self._processed_tweets:
                ...
                logging.debug(
                    f"Marked tweet {tweet_id} as unprocessed and saved state."
                )
            else:
                logging.debug(
    ...
    async def _validate_tweet_state_comprehensive(
        ...
        updates_needed = (
            {}
        )  # This will store changes identified within this validation run
        ...
        if has_media:
            for media_path_rel_str in media_list_rel:
                media_path_abs = self.config.resolve_path_from_project_root(
                    media_path_rel_str
                )
                if not media_path_abs.exists():
                    logging.warning(
                        f"Tweet {tweet_id} cache references missing media file: {media_path_rel_str} (resolved to {media_path_abs})"
                    )
        ...
        if not media_files_exist:
            ...
            not cache_complete
        ):  # Media files might exist, but cache wasn't marked complete.
            all_valid = False
        ...
        has_image_descriptions = bool(
            tweet_data.get("image_descriptions")
            and any(tweet_data.get("image_descriptions"))
        )
        ...
        if has_media:
            for media_path_rel_str in media_list_rel:
                if Path(media_path_rel_str).suffix.lower() in [
                    ".jpg",
                    ".jpeg",
                    ".png",
                    ".webp",
        ...
        if has_media:
            if (
                media_files_exist
                and (not has_images_needing_desc or has_image_descriptions)
                and not media_processed
            ):
                updates_needed["media_processed"] = True
                media_processed = True
            elif (
                media_files_exist
                and has_images_needing_desc
                and not has_image_descriptions
            ):
                if media_processed:
                    updates_needed["media_processed"] = False
                all_valid = False
        ...
        if not categories_processed:
            ...
            if not (main_cat and sub_cat and item_name):
                logging.warning(
                    f"Tweet {tweet_id} categories_processed but missing main fields: main='{main_cat}', sub='{sub_cat}', name='{item_name}'"
                )
                updates_needed["categories_processed"] = False
                all_valid = False
            else:
                is_default_pair = (main_cat, sub_cat) in default_category_tuples
                is_generic_item_name = (
                    item_name.startswith(f"Tweet-{tweet_id}")
                    or item_name.lower() == "tweet"
                    or len(item_name) < 5
                )
                ...
                if "categories" in tweet_data:
                    ...
                    if not (
                        cat_dict.get("main_category") == main_cat
                        and cat_dict.get("sub_category") == sub_cat
                        and cat_dict.get("item_name") == item_name
                    ):
                        logging.warning(
                            f"Tweet {tweet_id} has inconsistent categories between top-level fields and 'categories' dict"
                        )
                        updates_needed["categories_processed"] = False
                        all_valid = False

                # Check for the common error condition where all items are categorized the same
                api_design_count = sum(
                    1
                    for tid, tdata in self._tweet_cache.items()
                    if tdata.get("main_category") == "software_engineering"
                    and tdata.get("sub_category") == "api_design"
                    and tdata.get("item_name_suggestion")
                    == "restful_api_best_practices"
                )

                # If there are more than 10 items with identical "api_design" categories, likely a bug
                if (
                    main_cat == "software_engineering"
                    and sub_cat == "api_design"
                    and item_name == "restful_api_best_practices"
                    and api_design_count > 10
                ):
                    logging.warning(
                        f"Tweet {tweet_id} has suspicious common 'api_design' categorization (found {api_design_count} instances)"
                    )
                    updates_needed["categories_processed"] = False
                ...
                if (
                    is_default_pair
                    and is_generic_item_name
                    and recategorization_attempts < max_recategorization_attempts
                ):
                    logging.info(
                        f"Tweet {tweet_id} has default/generic category/name ({main_cat}/{sub_cat}/{item_name}), marking for re-categorization (attempt {recategorization_attempts + 1})"
                    )
                    updates_needed["categories_processed"] = False
                    updates_needed["main_category"] = (
                        ""  # Clear potentially bad categories
                    )
                    updates_needed["sub_category"] = ""
                    updates_needed["item_name_suggestion"] = ""
                    updates_needed["recategorization_attempts"] = (
                        recategorization_attempts + 1
                    )
                    all_valid = False
                elif (
                    is_default_pair
                    ...
                    logging.warning(
                        f"Tweet {tweet_id} reached max re-categorization attempts with generic category/name."
                    )
        ...
        if not kb_item_created:
            found_readme_rel_path = await self._find_and_update_kb_item_path(
                tweet_id, tweet_data
            )
            if found_readme_rel_path:
                ...
                logging.info(
                    f"Found existing KB item for tweet {tweet_id} at {found_readme_rel_path} (rel to project), marking created."
                )
            else:
                all_valid = False
        ...
        if kb_item_created:
            if not kb_path_rel_project_str:
                ...
                readme_abs_path = self.config.resolve_path_from_project_root(
                    kb_path_rel_project_str
                )
                if not readme_abs_path.exists():
                    logging.warning(
                        f"Tweet {tweet_id} KB item README (rel path {kb_path_rel_project_str}) not found at resolved {readme_abs_path}"
                    )
                    updates_needed["kb_item_created"] = False
        ...
        if updates_needed:
            ...
            data_to_update.update(
                updates_needed
            )  # Apply changes found in this validation
            ...
            await self._atomic_write_json(self._tweet_cache, self.tweet_cache_file)
            logging.debug(
                f"Updated tweet {tweet_id} state due to validation: {updates_needed}"
            )
            self.validation_fixes += 1
    ...
    async def _find_and_update_kb_item_path(
        ...
        main_cat = tweet_data.get("main_category") or tweet_data.get(
            "categories", {}
        ).get("main_category")
        sub_cat = tweet_data.get("sub_category") or tweet_data.get(
            "categories", {}
        ).get("sub_category")
        item_name = tweet_data.get("item_name_suggestion") or tweet_data.get(
            "categories", {}
        ).get("item_name")
        ...
        for item_dir_abs_path_hint in search_dirs_abs:
            if item_dir_abs_path_hint.is_dir():
                ...
                if readme_abs_path.exists():
                    try:
                        async with aiofiles.open(
                            readme_abs_path, "r", encoding="utf-8"
                        ) as f:
                            content_sample = await f.read(2048)
                            # Simple tweet ID validation - check if the specific tweet_id appears in the content
                            if tweet_id in content_sample:
                                logging.info(f"Found valid KB item for {tweet_id} via category hint at {readme_abs_path}")
                                return str(self.config.get_relative_path(readme_abs_path))
                    except Exception as e:
                        logging.warning(
                            f"Error reading potential README at {readme_abs_path} for tweet {tweet_id}: {e}"
                        )
                        continue
        ...
        logging.debug(
            f"KB item for {tweet_id} not found via hints, scanning all READMEs in {kb_base_abs}..."
        )
        for readme_abs_path_scan in kb_base_abs.rglob("README.md"):
            try:
                async with aiofiles.open(
                    readme_abs_path_scan, "r", encoding="utf-8"
                ) as f:
                    content_sample = await f.read(2048)
                    # Simple tweet ID validation - only return this path if it actually contains this specific tweet ID
                    if tweet_id in content_sample:
                        logging.info(
                            f"Found KB item for {tweet_id} via full scan at {readme_abs_path_scan}"
                        )
                        return str(self.config.get_relative_path(readme_abs_path_scan))
            except Exception as e:
                logging.warning(
                    f"Error reading potential README at {readme_abs_path_scan} during full scan for {tweet_id}: {e}"
                )
        return None

    async def validate_kb_items(self) -> None:
        """
        Validate KB items. Ensures that tweet_cache entries for kb_item_path correctly point to
        README.md files that correspond to the tweet_id and exist on disk.
        Resets kb_item_path and kb_item_created if inconsistencies are found.
        """
        logging.info("Validating KB items and cache consistency...")
        kb_base_abs = self.config.knowledge_base_dir
        updates = 0
        processed_readme_paths_for_current_validation = set() # To avoid double-processing if a file is somehow linked twice

        # Phase 1: Iterate through all READMEs on disk and update cache if they match a cached tweet.
        # This helps correct entries where the path might be wrong but a valid README exists.
        readme_files_on_disk = list(kb_base_abs.rglob("README.md"))
        logging.info(f"Found {len(readme_files_on_disk)} README.md files in {kb_base_abs} for validation.")

        for readme_abs_path in readme_files_on_disk:
            if readme_abs_path in processed_readme_paths_for_current_validation:
                continue
            
            readme_rel_project_path_str = str(self.config.get_relative_path(readme_abs_path))
            ...
            try:
                ...
                # Simple approach: look for any tweet IDs (18-19 digit numbers) in the content
                import re
                potential_tweet_ids = re.findall(r'\b\d{18,19}\b', content_sample)
                
                tweet_id_from_readme = None
                for potential_id in potential_tweet_ids:
                    if potential_id in self._tweet_cache:
                        tweet_id_from_readme = potential_id
                        break
                
                if tweet_id_from_readme:
                    processed_readme_paths_for_current_validation.add(readme_abs_path)

                    cached_tweet_data = self._tweet_cache[tweet_id_from_readme]
                    current_cached_path = cached_tweet_data.get("kb_item_path")
                    current_kb_created = cached_tweet_data.get("kb_item_created", False)

                    if current_cached_path != readme_rel_project_path_str or not current_kb_created:
                        logging.info(
                            f"Updating cache for Tweet {tweet_id_from_readme}: README at "
                            f"{readme_rel_project_path_str} matches. Old path: {current_cached_path}, old created: {current_kb_created}"
                        )
                        cached_tweet_data["kb_item_path"] = readme_rel_project_path_str
                        cached_tweet_data["kb_item_created"] = True
                        updates += 1
                    # else: KB item exists for a tweet not in cache - might be an orphaned file or a new item not yet through full flow.
                    # This function focuses on validating existing cache entries.
            except Exception as e:
                logging.warning(
                    f"Error reading or processing README at {readme_abs_path} during initial scan: {e}"
                )
                continue
        
        if updates > 0:
            logging.info(f"{updates} cache entries updated based on README scan.")
            # A save will happen at the end if total updates > 0


        # Phase 2: Iterate through the cache and validate each entry.
        logging.info(f"Validating {len(self._tweet_cache)} cached tweet entries against their KB item paths.")
        for tweet_id, tweet_data in list(self._tweet_cache.items()):
            original_kb_item_path = tweet_data.get("kb_item_path")
            original_kb_created = tweet_data.get("kb_item_created", False)
            needs_cache_reset = False
            log_prefix = f"Tweet {tweet_id}:"

            if original_kb_created and original_kb_item_path:
                kb_readme_abs = self.config.resolve_path_from_project_root(original_kb_item_path)
                
                if not kb_readme_abs.exists():
                    logging.warning(
                        f"{log_prefix} Cached kb_item_path {original_kb_item_path} (resolved: {kb_readme_abs}) does not exist. Resetting."
                    )
                    needs_cache_reset = True
                else:
                    # File exists, now verify its content matches the tweet_id
                    try:
                        async with aiofiles.open(kb_readme_abs, "r", encoding="utf-8") as f:
                            content_sample = await f.read(2048)
                        
                        # Simple tweet ID validation - just check if the tweet_id appears in the content
                        if tweet_id in content_sample:
                            # Tweet ID found in content, path is correct
                            pass  # No action needed for this tweet
                        else:
                            # README exists but does not contain the expected tweet ID
                            logging.warning(
                                f"{log_prefix} Cached kb_item_path {original_kb_item_path} exists, but tweet ID {tweet_id} not found in its content. Resetting."
                            )
                            needs_cache_reset = True
                    except Exception as e:
                        logging.warning(
                            f"{log_prefix} Error verifying tweet ID in README at {kb_readme_abs}: {e}. Resetting."
                        )
                        needs_cache_reset = True
            elif original_kb_created and not original_kb_item_path:
                # Marked created but path is empty - this is an inconsistent state.
                logging.warning(
                    f"{log_prefix} Marked kb_item_created=True but kb_item_path is empty. Resetting."
                )
                needs_cache_reset = True
            # If not original_kb_created, it's fine for path to be empty. No reset needed unless Phase 1 found a file.
            # If original_kb_path exists but original_kb_created is false, this is also an inconsistency handled by Phase 1 if file is valid.
            # If Phase 1 fixed it, original_kb_created would be true now. If not, it means the file was not found/valid for this tweet_id.


            if needs_cache_reset:
                if tweet_data.get("kb_item_path") != "" or tweet_data.get("kb_item_created") != False:
                    tweet_data["kb_item_path"] = ""
                    tweet_data["kb_item_created"] = False
                    updates += 1
                    logging.info(f"{log_prefix} Cache entry for KB item reset.")
            elif not original_kb_created and original_kb_item_path and tweet_id in self._tweet_cache and self._tweet_cache[tweet_id].get("kb_item_path") == original_kb_item_path and not self._tweet_cache[tweet_id].get("kb_item_created"):
                 # This case implies that Phase 1 did *not* validate this file for this tweet_id,
                 # but the cache has a path but is not marked created. This is an orphan path.
                 logging.warning(f"{log_prefix} Has kb_item_path {original_kb_item_path} but not marked created and not validated by disk scan. Resetting.")
                 tweet_data["kb_item_path"] = ""
                 # kb_item_created is already False
                 updates +=1


        if updates > 0:
            await self._atomic_write_json(
                self._tweet_cache, self.tweet_cache_file
            )  # Save updated cache
            logging.info(
                f"Persisted {updates} tweet cache updates from validate_kb_items."
            )
        else:
            logging.info("No cache updates required from validate_kb_items.")
    ...
    async def finalize_processing(self) -> None:
        ...
        for tweet_id in list(self._unprocessed_tweets):
            ...
            if not tweet_data:
                logging.warning(
                    f"Tweet {tweet_id} in unprocessed list but not in cache, removing from unprocessed."
                )
                if tweet_id in self._unprocessed_tweets:
                    self._unprocessed_tweets.remove(tweet_id)
                self.validation_fixes += 1
            ...
            is_fully_processed = await self._validate_tweet_state_comprehensive(
                tweet_id, tweet_data
            )
            if is_fully_processed:
                if tweet_id not in self._processed_tweets:  # Ensure not already marked
                    ...
                    logging.info(
                        f"Finalized tweet {tweet_id} and added to processed list."
                    )
                if (
                    tweet_id in self._unprocessed_tweets
                ):  # Remove from unprocessed if validation passed
                    self._unprocessed_tweets.remove(tweet_id)
            # If not fully_processed, it remains in unprocessed (or was put there by _validate_tweet_state_comprehensive)

        if (
            moved_to_processed > 0 or self.validation_fixes > 0
            ...
            await self._atomic_write_json(self._tweet_cache, self.tweet_cache_file)
            logging.info(
                f"Finalization complete: moved {moved_to_processed} tweets to processed. Total validation fixes during this phase: {self.validation_fixes}"
            )
    ...
    async def cleanup_unprocessed_tweets(self) -> None:
        ...
        for tweet_id in list(self._unprocessed_tweets):  # Iterate copy
            if tweet_id in self._processed_tweets:
                ...
                logging.debug(
                    f"Tweet {tweet_id} is already in processed list, scheduling removal from unprocessed."
                )
                continue
            ...
            if not tweet_data:
                to_remove_from_unprocessed.append(tweet_id)
                logging.debug(
                    f"Tweet {tweet_id} not found in cache, scheduling removal from unprocessed."
                )
                continue

            # Re-validate state comprehensively. If it passes, it should be moved to processed.
            is_fully_processed_and_valid = (
                await self._validate_tweet_state_comprehensive(tweet_id, tweet_data)
            )
            if is_fully_processed_and_valid:
                if tweet_id not in self._processed_tweets:  # Ensure not already marked
                    self._processed_tweets[tweet_id] = datetime.now(
                    ...
                    logging.info(
                        f"Tweet {tweet_id} validated as fully processed during cleanup, adding to processed list."
                    )
                    made_changes = True  # Processed list changed
                to_remove_from_unprocessed.append(
                    tweet_id
                )  # Remove from unprocessed if validation passed
        ...
        if made_changes:
            ...
            await self._atomic_write_json(
                self._tweet_cache, self.tweet_cache_file
            )  # Cache might have been updated by validation

        removed_count = initial_count - len(self._unprocessed_tweets)
        logging.info(
            f"Cleaned up {removed_count} tweets from unprocessed list. Current unprocessed: {len(self._unprocessed_tweets)}"
        )
    ...
    async def sync_knowledge_base_to_db(self, db_session=None):
        ...
        try:
            logging.info(
                "Starting DB sync from tweet cache. Paths are relative to project_root."
            )
            from flask import current_app
            from knowledge_base_agent.models import (
                db,
                KnowledgeBaseItem,
            )  # db is SQLAlchemy instance
            ...
            with current_app.app_context():
                ...
                if not actual_db_session:
                    logging.error(
                        "DB session not available for sync_knowledge_base_to_db"
                    )
                ...
                for tweet_id, tweet_data in self._tweet_cache.items():
                    if not (
                        tweet_data
                        and tweet_data.get("kb_item_created", False)
                        and tweet_data.get("kb_item_path")
                    ):
                        continue  # Skip if not marked as KB created or no path

                    total_eligible_from_cache += 1
                    kb_readme_path_rel_project = tweet_data[
                        "kb_item_path"
                    ]  # Path to README.md relative to project_root
                    kb_readme_abs_path = self.config.resolve_path_from_project_root(
                        kb_readme_path_rel_project
                    )

                    if not kb_readme_abs_path.exists():
                        logging.warning(
                            f"DB Sync: README.md missing for tweet {tweet_id} at {kb_readme_abs_path} (from rel {kb_readme_path_rel_project}). Skipping DB entry."
                        )
                        skipped_count += 1
                        continue
                    ...
                    try:
                        async with aiofiles.open(
                            kb_readme_abs_path, "r", encoding="utf-8"
                        ) as f:
                            content = await f.read()
                    except Exception as e:
                        logging.error(
                            f"DB Sync: Failed to read README.md for tweet {tweet_id} at {kb_readme_abs_path}: {e}"
                        )
                        readme_read_error_count += 1
                        skipped_count += 1
                        continue

                    title = tweet_data.get("item_name_suggestion") or tweet_data.get(
                        "categories", {}
                    ).get("item_name")
                    if not title:
                    ...
                    description = (
                        " ".join(readme_desc_lines)
                        if readme_desc_lines
                        else tweet_data.get("raw_data", {}).get(
                            "text", "No description available."
                        )[:250]
                    )

                    main_category = tweet_data.get("main_category") or tweet_data.get(
                        "categories", {}
                    ).get("main_category", "Uncategorized")
                    sub_category = tweet_data.get("sub_category") or tweet_data.get(
                        "categories", {}
                    ).get("sub_category", "General")
                    ...
                    source_url = tweet_data.get(
                        "url", f"https://twitter.com/i/web/status/{tweet_id}"
                    )
                    kb_media_paths_json = tweet_data.get(
                        "kb_media_paths", "[]"
                    )  # Already JSON string of paths rel to kb-generated

                    try:
                        existing_item = (
                            actual_db_session.query(KnowledgeBaseItem)
                        ...
                        if existing_item:
                            ...
                            new_item = KnowledgeBaseItem(
                                ...
                                file_path=file_path_to_store_in_db,
                        ...
                        logging.error(
                            f"DB Sync: DB operation (add/update) failed for tweet {tweet_id}: {e}"
                        )
                        db_op_error_count += 1
                        skipped_count += 1
                        continue
                ...
                if added_count > 0 or updated_count > 0:
                    try:
                        ...
                        logging.info(
                            f"DB Sync: Commit successful. Added: {added_count}, Updated: {updated_count}."
                        )
                    except Exception as e:
                        ...
                        raise StateManagerError(
                            f"DB Sync: Failed to commit KB items to DB: {e}"
                        )
                else:
                    logging.info("DB Sync: No new items or updates to commit to DB.")

                logging.info(
                    f"DB Sync Summary: Eligible from cache: {total_eligible_from_cache}, Added: {added_count}, Updated: {updated_count}, Skipped: {skipped_count} (Read Errors: {readme_read_error_count}, DB Op Errors: {db_op_error_count})"
                )

        except ImportError:
            logging.warning(
                "DB Sync: Flask or DB models not available. Skipping DB synchronization. This is normal if running outside Flask app context."
            )
        except Exception as e:
            ...
            if "actual_db_session" in locals() and actual_db_session:
                try:
                    actual_db_session.rollback()
                    logging.info("DB Sync: Rolled back session due to general error.")
                except:
                    pass  # Ignore rollback error if session is bad
            # Do not raise StateManagerError if it's just ImportError
            if not isinstance(e, ImportError):
                raise StateManagerError(f"DB Sync: Failed: {e}")
        ...
        try:
            ...
            if tweet_data.get('kb_item_created', False):
                ...
                try:
                    ...
                    if tweet_id not in content_preview:
    ...
    async def mark_phase_completed(self, tweet_id: str, phase: str) -> None:
        """
        Mark a specific processing phase as completed for a tweet.
        Phases: 'cache_complete', 'media_processed', 'categories_processed', 'kb_item_created', 'db_synced'
        """
        async with self._lock:
        tweet_data = await self.get_tweet(tweet_id)
        if tweet_data:
                tweet_data[phase] = True
                # Also add timestamp for when this phase was completed
                phase_timestamp_key = f"{phase}_timestamp"
                tweet_data[phase_timestamp_key] = datetime.now(timezone.utc).isoformat()
                
                await self.save_tweet_cache(tweet_id, tweet_data)
                logging.debug(f"Marked phase '{phase}' as completed for tweet {tweet_id}")
                
                # If this was the final phase, consider marking tweet as fully processed
                required_phases = ['cache_complete', 'media_processed', 'categories_processed', 'kb_item_created', 'db_synced']
                if all(tweet_data.get(phase_name, False) for phase_name in required_phases):
                    # Check if kb_item_path actually exists
                    kb_item_path_rel = tweet_data.get("kb_item_path")
                    if kb_item_path_rel:
                        ...
                        if kb_item_abs_path.exists():
                            ...
                            logging.info(f"Tweet {tweet_id} completed all phases and marked as fully processed")
        else:
                logging.warning(f"Cannot mark phase '{phase}' for tweet {tweet_id}: tweet not found in cache")

    async def get_processing_progress(self, tweet_id: str) -> Dict[str, Any]:
        """
        Get detailed processing progress for a tweet, including phase completion status and timestamps.
        """
        tweet_data = await self.get_tweet(tweet_id)
            if not tweet_data:
            return {"error": "Tweet not found in cache"}
        ...
        for phase in phases:
            progress["phases"][phase] = {
                "completed": tweet_data.get(phase, False),
                "timestamp": tweet_data.get(f"{phase}_timestamp")
            }
        
        # Calculate overall progress percentage
        completed_phases = sum(1 for phase in phases if tweet_data.get(phase, False))
        progress["progress_percentage"] = (completed_phases / len(phases)) * 100
